...
export default function MediaCard({ label, aspect = '16/9', kind = 'video', src, posterSrc, meta, className }: Props) {
  const aspectClass = aspect === '21/9' ? 'aspect-[21/9]' : 'aspect-video'
  const isVideo = kind === 'video' && !!src && src.toLowerCase().endsWith('.mp4')
  ...
  return (
    ...
    >
      <div className={`relative ${aspectClass} w-full`}>
        ...
        <div
          className={
            'absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(255,255,255,0.10),transparent_55%)] opacity-60' +
            (isVideo ? ' pointer-events-none' : '')
          }
        />
        ...
        <div
          className={
            'absolute left-4 top-4 inline-flex items-center gap-2 rounded-full border border-white/15 bg-black/30 px-3 py-1 text-xs font-semibold text-white backdrop-blur' +
            (isVideo ? ' pointer-events-none' : '')
          }
        >
          {label}
        ...
        {meta && (
          <div
            className={
              'absolute right-4 top-4 inline-flex items-center rounded-full border border-white/15 bg-black/30 px-3 py-1 text-xs font-semibold text-white/90 backdrop-blur' +
              (isVideo ? ' pointer-events-none' : '')
            }
          >
            {meta}
